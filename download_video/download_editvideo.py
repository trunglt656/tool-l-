...
def send_video_to_phone(video_path):
    try:
        result = subprocess.run(["adb", "devices"], capture_output=True, text=True)
        if "device" not in result.stdout:
            print("Lỗi: Không tìm thấy thiết bị Android. Hãy chắc chắn thiết bị đã kết nối và bật USB Debugging.")
            return
...
def add_product_description(description):
    print("Đang nhập mô tả sản phẩm...")
    run_adb_command("adb shell input tap 593 417")  # Nhấn vào ô mô tả sản phẩm
    time.sleep(2)
    run_adb_command(f"adb shell input text '{description}'")  # Nhập mô tả
    time.sleep(2)

# Tải video TikTok và ghép nhạc
def start_process():
    video_url = url_entry.get()
    audio_file_path = audio_entry.get()
    product_link = product_link_entry.get()
