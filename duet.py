...
def check_cart_icon_images():
    ...
    if max_val1 > 0.8 or max_val2 > 0.6:  # Ngưỡng khớp mẫu, có thể điều chỉnh
        if max_val1 > 0.8:
...
button_text = "Duet"  # Thay bằng tên nút bạn muốn kiểm tra
if check_button_exists(button_text):
    print("Chức năng 'Duet' tồn tại. Tiến hành kiểm tra giỏ hàng.")
    time.sleep(4)
    ...
    if check_cart_icon_images():  # Gọi hàm kiểm tra với hai hình
        ...
        run_adb_command("adb shell input swipe 712 2183 261 2183 500")
        time.sleep(3)
        run_adb_command("adb shell input tap 596 2160")  # Nhấn duet button
        time.sleep(4)

        # chon bố cục - > phông xanh
        run_adb_command("adb shell input tap 1008 1078")
        run_adb_command("adb shell input tap 940 2142")
        # thoát để quay
        run_adb_command("adb shell input tap 561 778")

        # quay
        run_adb_command("adb shell input swipe 527 1848 527 1848 30000")
        ...
        time.sleep(8)
        # run_adb_command('adb shell input tap 858 1853')# ket thuc ghi hinh
...
# quay lai
        ...
        run_adb_command("adb shell input tap 955 160")
        time.sleep(9)
        run_adb_command("adb shell input swipe 531 1853 531 1853 1000") #ghi hinhf
        time.sleep(5)
        ...
        time.sleep(6)
        run_adb_command('adb shell input tap 971 143') #link 
        time.sleep(5)
